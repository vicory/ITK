...
namespace itk
{
  ...
  class TBBFunctor
    ...
    public:
      ...
      typedef ImageToImageFilter<TInputImage,TOutputImage> TbbImageFilterType;

      TBBFunctor(TbbImageFilterType *tbbFilter):
      m_TbbFilter(tbbFilter) {}

      void operator() ( const tbb::blocked_range<int>& r ) const
      {
        int threadId = r.begin();

        typename TOutputImage::RegionType myRegion;
        unsigned int total = m_TbbFilter->SplitRequestedRegion(threadId, m_TbbFilter->GetNumberOfThreads(), myRegion);

        // Run the ThreadedGenerateData method!
        if (threadId < total)
          m_TbbFilter->ThreadedGenerateData(myRegion, threadId);
                                                                        ...
                                                                        this->m_DirectionTolerance))
        {
                                                                          ...
                                                                          this->m_DirectionTolerance))
          {
            ...
            directionString << "InputImage Direction: " << inputPtr1->GetDirection()
                            << ", InputImage" << it.GetName() << " Direction: " << inputPtrN->GetDirection()
                            << std::endl;
  ...
  //  return outputSize[OutputImageDimension - 1];
  //}

  template< typename TInputImage, typename TOutputImage >
  void
  ImageToImageFilter<TInputImage, TOutputImage>
  ::UseOldBehavior(bool UseOldBehavior)
  {
    m_UseOldBehavior = UseOldBehavior;
  }
  ...
  ::GenerateData()
  {
    ...
    if (m_UseOldBehavior)
    {
      this->m_NumberOfJobs = this->GetNumberOfThreads();
    }
    else
    {
        this->m_NumberOfJobs = tbb::task_scheduler_init::default_num_threads();
        this->SetNumberOfThreads(15*this->m_NumberOfJobs);
    }
      ...
      this->SetNumberOfThreads(validThreads);

      if (m_UseOldBehavior)
        tbb::task_scheduler_init init(this->m_NumberOfJobs);

      // Do the task decomposition using parallel_for
      tbb::parallel_for( tbb::blocked_range<int>(0, this->GetNumberOfThreads(), 1),
        TBBFunctor<TInputImage, TOutputImage>(this));
